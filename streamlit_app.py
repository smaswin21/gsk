    ...
    with left_col:
        ...
        with st.form("calculator_form_main"):
            total_6m_sales = st.number_input(
                ...
                min_value=1, step=25,
                value=int(st.session_state.get("total_6m_sales", bundle["default_raw_inputs"]["total_6m_sales"])),
...
def render_data_explorer(bundle: dict[str, Any]) -> None:
    ...
    with col_dist_sel:
        ...
        for ind_opt in ["A", "B", "C"]:
            ...
            label = f"{dot} {ind_opt}"
            st.markdown(
                f'<div style="margin-bottom:0.3rem;padding:0.45rem 0.6rem;'
                f'border-radius:8px;border:2px solid {hex_color};'
                f'background:{hex_color if is_on else "transparent"};'
                f'color:{"white" if is_on else hex_color};'
                f'font-weight:700;font-size:0.95rem;text-align:center;'
